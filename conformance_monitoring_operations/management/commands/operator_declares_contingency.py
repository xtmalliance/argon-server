...
from scd_operations.scd_data_definitions import Time, OperationalIntentReferenceDSSResponse, ImplicitSubscriptionParameters, LatLngPoint, Polygon, Volume4D
    ...
    def handle(self, *args, **options):
        ...
        if r.exists(flight_opint):
            ...
            if not dry_run:                 
                ...
                operational_update_response = my_scd_dss_helper.update_specified_operational_intent_reference(subscription_id= subscription_id, operational_intent_ref_id = reference.id,extents= stored_volumes, new_state= str(contingent_state),ovn= reference.ovn, get_airspace_keys= False)
                ...
                if aircraft_bounds_conformant: # Operator declares contingency, but the aircraft is within bounds, no need to update / change bounds
                    ...
                    newv4D = my_op_int_converter.buffer_point_to_volume4d(lat = lat_dd, lon_dd= lon_dd, start_datetime=flight_declaration.start_datetime, end_datetime=flight_declaration.end_datetime)
